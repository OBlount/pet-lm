...
def keep_basic_punctuation(text):
    white_list = { ".", ",", "'", "?" }
    black_list = set(string.punctuation) - white_list
    return "".join([char for char in text if char not in black_list])
...
def filter_stop_words(tokens):
    ...
    return [(token, pos) for token, pos in tokens if token not in stop_words]
